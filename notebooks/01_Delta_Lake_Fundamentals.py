...
# MAGIC %md
# MAGIC # Delta Lake Fundamentals: Concepts #1-#10
# MAGIC
# MAGIC ## Notebook Overview
# MAGIC
# MAGIC This notebook covers the **10 core concepts** of Delta Lake, from ACID transactions to advanced table configuration. Each concept includes a problem statement, real-world use case, hands-on code demo, key takeaways, and a self-assessment question.
# MAGIC
# MAGIC **Environment:** Designed for Databricks Community Edition (free tier) — single node, no Unity Catalog, no Photon, no serverless.
# MAGIC
# MAGIC **Concepts Covered:**
# MAGIC 1. ACID Transactions [Easy]
# MAGIC 2. Transaction Log (_delta_log) [Easy]
# MAGIC 3. Time Travel & RESTORE [Easy]
# MAGIC 4. Schema Enforcement vs. Evolution [Medium]
# MAGIC 5. Liquid Clustering [Medium]
# MAGIC 6. OPTIMIZE & File Compaction [Medium]
# MAGIC 7. VACUUM & Storage Lifecycle [Medium]
# MAGIC 8. Change Data Feed (CDF) [Medium]
# MAGIC 9. Deletion Vectors [Medium]
# MAGIC 10. Delta Table Properties & Configuration [Hard]
# MAGIC
# MAGIC ---
...
# MAGIC %md
# MAGIC ## Setup: Create Working Directory & Clean Up
# MAGIC
# MAGIC We'll use a dedicated directory under `/tmp/` for all our Delta tables. This keeps things organised and makes cleanup easy.
...
# MAGIC %md
# MAGIC ## Concept #1: ACID Transactions [Easy]
# MAGIC
# MAGIC ### What Problem It Solves
# MAGIC
# MAGIC In traditional data lakes (Parquet, CSV, JSON), a failed write can leave the data in a corrupt or incomplete state. If a Spark job crashes mid-write, you might have partial files that downstream readers interpret as valid data. There&apos;s no built-in mechanism to ensure that a set of operations either all succeed or all fail as one unit.
# MAGIC
# MAGIC Delta Lake solves this by providing **full ACID transactions** on your data lake:
# MAGIC - **Atomicity:** Multiple writes are committed together or fully rolled back.
# MAGIC - **Consistency:** Readers always see a consistent snapshot of the data.
# MAGIC - **Isolation:** Concurrent writes are serialised via optimistic concurrency control.
# MAGIC - **Durability:** Once committed, data persists.
# MAGIC
# MAGIC ### Real-World Use Case
# MAGIC
# MAGIC An e-commerce platform writes order data and updates inventory levels in the same transaction. If the inventory update fails, the order write must also be rolled back to prevent data inconsistency.
# MAGIC
# MAGIC ### Hands-On Demo
...
# MAGIC %md
# MAGIC #### Atomic Write: Multiple Operations in One Transaction
# MAGIC
# MAGIC We&apos;ll perform an INSERT and an UPDATE as a single atomic transaction using `MERGE`. If anything fails, nothing gets committed.
...
# MAGIC %md
# MAGIC #### Demonstrating Rollback on Failure
# MAGIC
# MAGIC What happens if a write fails? With raw Parquet, partial files are left behind. With Delta, the transaction is rolled back completely. Let&apos;s demonstrate by writing to a raw Parquet directory at the same path to show the difference, then show Delta&apos;s safe behaviour.
...
# MAGIC %md
# MAGIC #### Demonstrating Optimistic Concurrency Control
# MAGIC
# MAGIC Delta uses optimistic concurrency by detecting conflicts at commit time. If two writers attempt conflicting modifications, one will be rejected with a `ConcurrentAppendException` or `ConcurrentDeleteReadException`.
...
# MAGIC %md
# MAGIC ### Key Takeaways
# MAGIC - Delta Lake guarantees **atomicity**: all operations in a transaction succeed or fail together.
# MAGIC - **Optimistic concurrency control** allows high-throughput concurrent writes without locking.
# MAGIC - Unlike raw Parquet/CSV, a failed Delta write leaves **no partial or corrupt data**.
# MAGIC
# MAGIC ### Self-Assessment Question
# MAGIC
# MAGIC *Q: What happens if you have a `MERGE` statement that inserts 100 rows and updates 50 rows, and it fails halfway through?*
# MAGIC
# MAGIC <details><summary>Click for answer</summary>
# MAGIC **A:** With Delta Lake, nothing is committed. All 100 inserts and 50 updates are rolled back. With raw Parquet, you could end up with partial files written, corrupting your downstream reads.
# MAGIC </details>
...
# MAGIC %md
# MAGIC ---
# MAGIC ## Concept #2: Transaction Log (_delta_log) [Easy]
# MAGIC
# MAGIC ### What Problem It Solves
# MAGIC
# MAGIC How does Delta know which files belong to which version of the table? Without a transaction log, there&apos;s no way to track which files were added or removed over time, no way to support time travel, and no way to ensure snapshot isolation for concurrent readers and writers.
# MAGIC
# MAGIC The `_delta_log` directory is the **source of truth** for a Delta table. It contains ordered JSON files (and checkpoint Parquet files) that record every operation performed on the table.
# MAGIC
# MAGIC ### Real-World Use Case
# MAGIC
# MAGIC A data engineering team needs to audit who modified a critical financial table and when. The transaction log provides a complete, immutable audit trail of every write operation.
# MAGIC
# MAGIC ### Hands-On Demo
...
# MAGIC %md
# MAGIC #### Inspect the `_delta_log` Directory
# MAGIC
# MAGIC Every Delta table has a `_delta_log` subdirectory containing JSON transaction log entries and, after enough operations, checkpoint Parquet files.
...
# MAGIC %md
# MAGIC #### View Transaction History with `DESCRIBE HISTORY`
...
# MAGIC %md
# MAGIC #### Checkpoint Creation
# MAGIC
# MAGIC After many operations (default: every 10 commits), Delta creates a **checkpoint** — a Parquet file that aggregates all previous JSON entries. This speeds up table state reconstruction by not requiring replay of the entire log.
...
# MAGIC %md
# MAGIC ### Key Takeaways
# MAGIC - The **`_delta_log` directory** is the single source of truth for a Delta table&apos;s state.
# MAGIC - JSON log files record every **add**, **remove**, and **metadata change** as ordered transactions.
# MAGIC - **Checkpoints** (Parquet files) are created every 10 commits by default, enabling fast snapshot reads without replaying the entire log.
# MAGIC
# MAGIC ### Self-Assessment Question
# MAGIC
# MAGIC *Q: If you delete all `.json` files from the `_delta_log` but keep the `.checkpoint.parquet` file, can the table still be read?*
# MAGIC
# MAGIC <details><summary>Click for answer</summary>
# MAGIC **A:** You can read up to the checkpoint version, but any versions AFTER the checkpoint will be lost because the JSON files containing those incremental changes are gone. Always treat the `_delta_log` directory as immutable — never manually delete entries!
# MAGIC </details>
...
# MAGIC %md
# MAGIC ---
# MAGIC ## Concept #3: Time Travel & RESTORE [Easy]
# MAGIC
# MAGIC ### What Problem It Solves
# MAGIC
# MAGIC Data engineers often need to query data &quot;as it was&quot; at a specific point in the past — for auditing, debugging bad writes, reproducing reports, or rolling back accidental changes. Without built-in versioning, you&apos;d have to maintain costly snapshot copies of your data.
# MAGIC
# MAGIC Delta Lake stores a complete version history, allowing you to query any past table version or restore the table to a previous state.
# MAGIC
# MAGIC ### Real-World Use Case
# MAGIC
# MAGIC A scheduled job accidentally runs a `DELETE` that removes 2 million valid customer records. Instead of restoring from a backup, the team simply runs `RESTORE TABLE` to roll back to the version just before the bad job ran.
# MAGIC
# MAGIC ### Hands-On Demo
...
# MAGIC %md
# MAGIC #### Query with `VERSION AS OF` and `TIMESTAMP AS OF`
...
# MAGIC %md
# MAGIC #### Demonstrate `RESTORE TABLE` to Roll Back
...
# MAGIC %md
# MAGIC #### Time Travel with `TIMESTAMP AS OF`
...
# MAGIC %md
# MAGIC #### Retention Period and VACUUM Interaction
# MAGIC
# MAGIC Delta&apos;s default retention for time travel is **7 days** (controlled by `delta.logRetentionDuration`). After that, VACUUM can remove old files, and time travel beyond the retention window will fail. We&apos;ll explore this further in Concept #7.
...
# MAGIC %md
# MAGIC ### Key Takeaways
# MAGIC - `VERSION AS OF N` lets you query **any past version** of a Delta table without maintaining snapshots.
# MAGIC - `RESTORE TABLE` rolls the table back to a previous version, creating a **new version** that mirrors the old state.
# MAGIC - Time travel is limited by the **retention period** (default 7 days for files); VACUUM removes expired files permanently.
# MAGIC
# MAGIC ### Self-Assessment Question
# MAGIC
# MAGIC *Q: You run `RESTORE TABLE orders TO VERSION AS OF 5`. Is version 6 lost forever?*
# MAGIC
# MAGIC <details><summary>Click for answer</summary>
# MAGIC **A:** No! `RESTORE` creates a **new version** (e.g., version 7) that copies the state from version 5. Version 6 still exists in the transaction log and you can still time-travel to it — unless VACUUM removes the underlying data files after the retention period expires.
# MAGIC </details>
...
# MAGIC %md
# MAGIC ---
# MAGIC ## Concept #4: Schema Enforcement vs. Evolution [Medium]
# MAGIC
# MAGIC ### What Problem It Solves
# MAGIC
# MAGIC Traditional data lakes (Parquet, CSV) have **no schema enforcement**: you can accidentally write malformed data, wrong column types, or extra/missing columns. This creates silent data quality issues that are discovered only when downstream jobs fail.
# MAGIC
# MAGIC Delta Lake **enforces schema** on write by default, preventing bad data from entering the table. At the same time, Delta provides **schema evolution** (`mergeSchema`) so you can explicitly add new columns without breaking existing pipelines.
# MAGIC
# MAGIC ### Real-World Use Case
# MAGIC
# MAGIC A new field (`customer_tier`) is added to the source system. The engineering team enables `mergeSchema` to let the new column be added automatically to the Delta table, avoiding pipeline failures while preserving all existing data.
# MAGIC
# MAGIC ### Hands-On Demo
...
# MAGIC %md
# MAGIC #### Schema Enforcement: Rejecting Bad Data
...
# MAGIC %md
# MAGIC #### Schema Evolution: `mergeSchema`
...
# MAGIC %md
# MAGIC #### `autoMerge` in Writes
# MAGIC
# MAGIC You can set `spark.databricks.delta.schema.autoMerge.enabled = true` at the SparkSession level to make schema evolution opt-out instead of opt-in.
...
# MAGIC %md
# MAGIC ### Key Takeaways
# MAGIC - **Schema enforcement** prevents bad data (wrong types, extra/missing columns) from entering your Delta table — a critical data quality guard.
# MAGIC - **`mergeSchema`** allows intentional schema evolution by adding new columns without losing data.
# MAGIC - **`overwriteSchema`** replaces the entire schema (and data) — use with caution.
# MAGIC
# MAGIC ### Self-Assessment Question
# MAGIC
# MAGIC *Q: Your upstream source adds a new field `loyalty_points`. What&apos;s the safest way to add this column to your existing Delta table without breaking downstream consumers?*
# MAGIC
# MAGIC <details><summary>Click for answer</summary>
# MAGIC **A:** Use `mergeSchema` (or set `autoMerge`). This adds the new column with NULL values for existing rows. Downstream consumers that don&apos;t reference the new column are unaffected. `overwriteSchema` would be dangerous because it replaces ALL data.
# MAGIC </details>
...
# MAGIC %md
# MAGIC ---
# MAGIC ## Concept #5: Liquid Clustering [Medium]
# MAGIC
# MAGIC ### What Problem It Solves
# MAGIC
# MAGIC Traditional data partitioning requires you to choose a partition column upfront (e.g., `date`). If you choose wrong (too many small partitions, or data skew), you get terrible performance with no easy fix. Z-Ordering helps with multi-dimensional clustering but requires manual OPTIMIZE commands.
# MAGIC
# MAGIC **Liquid Clustering** is Delta&apos;s next-generation clustering technique that replaces both partitioning and Z-Ordering. It incrementally clusters data as it&apos;s written and dynamically adjusts to changing data patterns — no manual maintenance.
# MAGIC
# MAGIC **IMPORTANT NOTE FOR COMMUNITY EDITION:** Liquid Clustering requires `delta.minReaderVersion >= 3` and `delta.minWriterVersion >= 7`, which may not be available in all Community Edition deployments. If the syntax fails, this section explains the concept and shows traditional Z-Ordering as the closest equivalent.
# MAGIC
# MAGIC ### Real-World Use Case
# MAGIC
# MAGIC An IoT sensor table receives 500M rows/hour with timestamps, device IDs, and sensor types. Queries often filter by `sensor_type` AND `device_id`. Partitioning by either alone causes skew. Liquid Clustering clusters on both columns automatically, providing efficient file skipping for all query patterns.
# MAGIC
# MAGIC ### Hands-On Demo
...
# MAGIC %md
# MAGIC #### Traditional Z-Ordering Approach (Available in Community Edition)
...
# MAGIC %md
# MAGIC #### Liquid Clustering Syntax (if available in your environment)
# MAGIC
# MAGIC The `CLUSTER BY` syntax creates a table with Liquid Clustering enabled. If your Community Edition supports it, this cell will execute. If not, it will fail gracefully.
...
# MAGIC %md
# MAGIC #### Comparing Query Performance (File Skipping)
...
# MAGIC %md
# MAGIC ### Key Takeaways
# MAGIC - **Liquid Clustering** replaces traditional partitioning and Z-Ordering with incremental, maintenance-free clustering.
# MAGIC - Use `CLUSTER BY (col1, col2)` on table creation (requires compatible Runtime version).
# MAGIC - In environments without Liquid Clustering, **Z-Ordering via `OPTIMIZE ZORDER BY`** provides similar multi-dimensional clustering benefits.
# MAGIC
# MAGIC ### Self-Assessment Question
# MAGIC
# MAGIC *Q: When would you use Liquid Clustering over traditional `PARTITIONED BY`?*
# MAGIC
# MAGIC <details><summary>Click for answer</summary>
# MAGIC **A:** Use Liquid Clustering when you have high-cardinality columns (e.g., device IDs, user IDs) that would create too many small partitions, or when query patterns filter by multiple different columns. Liquid Clustering handles multi-dimensional clustering automatically without manual partition tuning.
# MAGIC </details>
...
# MAGIC %md
# MAGIC ---
# MAGIC ## Concept #6: OPTIMIZE &amp; File Compaction [Medium]
# MAGIC
# MAGIC ### What Problem It Solves
# MAGIC
# MAGIC Streaming and frequent small-batch writes create many **small files**. This &quot;small file problem&quot; degrades read performance because the query engine must open, read metadata from, and close thousands of tiny files.
# MAGIC
# MAGIC `OPTIMIZE` consolidates small files into larger ones (~1 GB by default), dramatically improving read performance. Combined with `ZORDER BY`, it also clusters related data together for file skipping.
# MAGIC
# MAGIC ### Real-World Use Case
# MAGIC
# MAGIC A Kafka streaming pipeline writes 50MB micro-batches every minute into a Delta table. After 24 hours, there are 1,440 small files. Running `OPTIMIZE` hourly consolidates them into larger files, keeping query performance predictable.
# MAGIC
# MAGIC ### Hands-On Demo
...
# MAGIC %md
# MAGIC #### File Count Before OPTIMIZE
...
# MAGIC %md
# MAGIC #### File Count After OPTIMIZE
...
# MAGIC %md
# MAGIC #### Read Performance Impact
# MAGIC
# MAGIC After OPTIMIZE, queries that filter on the Z-Ordered column benefit from file skipping — only relevant files are read, reducing scan overhead significantly.
...
# MAGIC %md
# MAGIC ### Key Takeaways
# MAGIC - `OPTIMIZE` consolidates small files into **larger files** (target ~1 GB), reducing metadata overhead and improving read performance.
# MAGIC - `OPTIMIZE ... ZORDER BY (col)` additionally **co-locates related data** in the same files, enabling file skipping.
# MAGIC - In production, run OPTIMIZE **periodically** (e.g., hourly or daily) to keep the file count manageable — it&apos;s idempotent and safe to run on live tables.
# MAGIC
# MAGIC ### Self-Assessment Question
# MAGIC
# MAGIC *Q: Your table has 10,000 small files after a day of streaming writes. Running `OPTIMIZE` consolidates them to 10 files. What happens to the old small files?*
# MAGIC
# MAGIC <details><summary>Click for answer</summary>
# MAGIC **A:** The old small files are marked as &quot;removed&quot; in the transaction log. They are NOT deleted immediately — they remain until VACUUM runs after the retention period (default 7 days). This is how Delta supports time travel even after OPTIMIZE.
# MAGIC </details>
...
# MAGIC %md
# MAGIC ---
# MAGIC ## Concept #7: VACUUM &amp; Storage Lifecycle [Medium]
# MAGIC
# MAGIC ### What Problem It Solves
# MAGIC
# MAGIC Over time, Delta tables accumulate old data files that are no longer referenced by the latest table version (from OPTIMIZE, DELETE, UPDATE operations). These orphaned files waste storage and cost money.
# MAGIC
# MAGIC `VACUUM` removes unreferenced files older than the retention period (default 7 days). This is the Delta equivalent of garbage collection — essential for managing storage costs.
# MAGIC
# MAGIC ### Real-World Use Case
# MAGIC
# MAGIC A data engineering team runs weekly `OPTIMIZE` on a 10 TB sales table. Without VACUUM, the unreferenced files from each week&apos;s OPTIMIZE would accumulate, doubling or tripling storage costs. With `VACUUM`, only the current and recent versions&apos; files are retained.
# MAGIC
# MAGIC ### Hands-On Demo
...
# MAGIC %md
# MAGIC #### `DESCRIBE DETAIL` — See Storage Statistics
...
# MAGIC %md
# MAGIC #### Check Retention Period and Version History
...
# MAGIC %md
# MAGIC #### Run VACUUM (with Dry Run)
# MAGIC
# MAGIC `VACUUM` with `DRY RUN` shows which files would be removed without actually deleting them. Always run a dry run first in production!
...
# MAGIC %md
# MAGIC #### Run VACUUM (Retain 0 Hours for Demo)
# MAGIC
# MAGIC **WARNING:** Setting retention to 0 hours disables time travel to previous versions. This is for demonstration only. In production, use the default 7-day retention.
...
# MAGIC %md
# MAGIC #### Time Travel AFTER VACUUM — What Happens?
...
# MAGIC %md
# MAGIC ### Key Takeaways
# MAGIC - `VACUUM` removes old data files that are no longer referenced within the **retention period** (default 7 days).
# MAGIC - Always run `DRY RUN` first to preview which files will be deleted.
# MAGIC - After VACUUM, **time travel** to versions older than the retention period will fail. Plan your retention window based on audit/compliance needs.
# MAGIC
# MAGIC ### Self-Assessment Question
# MAGIC
# MAGIC *Q: You set `delta.deletedFileRetentionDuration = 1 hour` and run VACUUM. Two hours later, can you query the table as it was 30 minutes ago?*
# MAGIC
# MAGIC <details><summary>Click for answer</summary>
# MAGIC **A:** No. VACUUM would have removed files older than 1 hour. After 2 hours, the 30-minutes-ago version is within the retention window, but the version from 2.5 hours ago would be gone. Always set retention to cover your maximum time-travel needs plus a safety margin.
# MAGIC </details>
...
# MAGIC %md
# MAGIC ---
# MAGIC ## Concept #8: Change Data Feed (CDF) [Medium]
# MAGIC
# MAGIC ### What Problem It Solves
# MAGIC
# MAGIC How do you efficiently capture row-level changes (inserts, updates, deletes) from a Delta table for downstream incremental processing? Without CDF, you&apos;d have to do expensive full-table comparisons or rely on unreliable &quot;last modified&quot; columns.
# MAGIC
# MAGIC **Change Data Feed (CDF)** records every row-level change made to a Delta table, including the type of change (`insert`, `update_preimage`, `update_postimage`, `delete`) and the version at which it occurred. This unlocks efficient CDC (Change Data Capture) pipelines.
# MAGIC
# MAGIC ### Real-World Use Case
# MAGIC
# MAGIC A data warehouse team needs to sync a Delta table to a downstream analytics database (e.g., PostgreSQL) in near-real-time. Instead of re-syncing the entire table, they read only the changes since the last sync via CDF.
# MAGIC
# MAGIC ### Hands-On Demo
...
# MAGIC %md
# MAGIC #### Perform Multiple Changes to Generate CDF Entries
...
# MAGIC %md
# MAGIC #### Read Change Data Feed with `table_changes()`
...
# MAGIC %md
# MAGIC #### Understanding `_change_type` Values
...
# MAGIC %md
# MAGIC #### Incremental Processing Pattern with CDF
...
# MAGIC %md
# MAGIC ### Key Takeaways
# MAGIC - CDF records every row-level change with `_change_type`, `_commit_version`, and `_commit_timestamp`.
# MAGIC - Use `table_changes('table_name', start_version, end_version)` to query changes efficiently for incremental processing.
# MAGIC - UPDATE operations produce **both** `update_preimage` (old values) and `update_postimage` (new values) — essential for accurate CDC replication.
# MAGIC
# MAGIC ### Self-Assessment Question
# MAGIC
# MAGIC *Q: You enable CDF on an existing table. Can you query changes that happened BEFORE CDF was enabled?*
# MAGIC
# MAGIC <details><summary>Click for answer</summary>
# MAGIC **A:** No. CDF only captures changes from the version where it was enabled and onwards. If you need change data for historical versions, you&apos;d need to use time travel to compare versions manually, which is possible but less efficient.
# MAGIC </details>
...
# MAGIC %md
# MAGIC ---
# MAGIC ## Concept #9: Deletion Vectors [Medium]
# MAGIC
# MAGIC ### What Problem It Solves
# MAGIC
# MAGIC In traditional Delta Lake, a `DELETE` operation physically rewrites all files containing matching rows — a costly, I/O-intensive operation. For a 1 TB table, deleting 10 rows could require rewriting hundreds of GB of data just to exclude those rows.
# MAGIC
# MAGIC **Deletion Vectors** (DVs) solve this by using **soft-delete markers** stored in separate, small binary files. When a DELETE runs, instead of rewriting data files, Delta records which rows to skip in a deletion vector file. This makes DELETE operations near-instant, regardless of table size.
# MAGIC
# MAGIC **NOTE FOR COMMUNITY EDITION:** Deletion Vectors require `delta.minReaderVersion >= 3` and `delta.minWriterVersion >= 7`. If these features aren&apos;t available in your Community Edition deployment, this section demonstrates the concept and shows the performance difference by simulating the before/after behavior.
# MAGIC
# MAGIC ### Real-World Use Case
# MAGIC
# MAGIC A GDPR compliance job needs to delete specific user records from a 5 TB events table. Without deletion vectors, each DELETE would take hours and cost hundreds of dollars in compute. With deletion vectors, the DELETE completes in seconds and the file rewrite cost is avoided — the soft-deleted rows are physically removed later during OPTIMIZE.
# MAGIC
# MAGIC ### Hands-On Demo
...
# MAGIC %md
# MAGIC #### Deletion Vectors — Enable and Demonstrate (if available)
...
# MAGIC %md
# MAGIC #### How Deletion Vectors Work — Architectural Overview
# MAGIC
# MAGIC **Without Deletion Vectors (Traditional):**
# MAGIC 1. DELETE identifies files containing matching rows
# MAGIC 2. Each matching file is **fully rewritten** minus the deleted rows
# MAGIC 3. Old files are removed from the log, new files are added
# MAGIC 4. Cost: O(file_size) for each file touched
# MAGIC
# MAGIC **With Deletion Vectors:**
# MAGIC 1. DELETE identifies specific row positions in each file
# MAGIC 2. A small **deletion vector** (binary file) is created, recording which rows to skip
# MAGIC 3. Data files are **NOT rewritten** — they remain in place
# MAGIC 4. Readers filter out soft-deleted rows at read time
# MAGIC 5. Cost: O(1) for the DELETE, O(deleted_rows) to filter during reads
# MAGIC 6. **Physical compaction** happens during OPTIMIZE (reclaims space)

# COMMAND ----------

# MAGIC %md
# MAGIC #### Demonstrate Soft-Delete vs Physical Delete Behavior
...
# MAGIC %md
# MAGIC #### View Deletion Vector Stats (if available)
...
# MAGIC %md
# MAGIC ### Key Takeaways
# MAGIC - **Deletion Vectors** enable near-instant DELETEs by using soft-delete markers instead of rewriting files.
# MAGIC - Without DVs, a DELETE rewrites **every file** that contains matching rows — costly for large tables.
# MAGIC - Soft-deleted rows are **filtered at read time** and physically removed during OPTIMIZE/VACUUM.
# MAGIC
# MAGIC ### Self-Assessment Question
# MAGIC
# MAGIC *Q: You enable deletion vectors and run `DELETE FROM transactions WHERE user_id = 'GDPR_Subject_123'`. The user queries the table and doesn&apos;t see their data. Is the data still on disk?*
# MAGIC
# MAGIC <details><summary>Click for answer</summary>
# MAGIC **A:** Yes, the data is still physically on disk (soft-deleted) and may remain there until OPTIMIZE physically removes it. The deletion vector ensures the data is filtered out at read time, so the user cannot see it — but for true GDPR compliance, you should ensure the data is physically removed via OPTIMIZE + VACUUM.
# MAGIC </details>
...
# MAGIC %md
# MAGIC ---
# MAGIC ## Concept #10: Delta Table Properties &amp; Configuration [Hard]
# MAGIC
# MAGIC ### What Problem It Solves
# MAGIC
# MAGIC Delta Lake has dozens of configurable properties that control behaviour: auto-optimization, column mapping, protocol versions, retention, isolation levels, and more. Without understanding these properties, you can&apos;t tune Delta for production workloads — you might miss out on auto-compaction, column renaming, or proper isolation configuration.
# MAGIC
# MAGIC ### Real-World Use Case
# MAGIC
# MAGIC A team needs to rename a column in a 10 TB Delta table without rewriting any data. By enabling `delta.columnMapping.mode = 'name'`, they can rename columns via `ALTER TABLE ... RENAME COLUMN` instantly — a metadata-only operation.
# MAGIC
# MAGIC ### Hands-On Demo
...
# MAGIC %md
# MAGIC #### 1. `SHOW TBLPROPERTIES` — View All Properties
...
# MAGIC %md
# MAGIC #### 2. Key Production Properties Explained
...
# MAGIC %md
# MAGIC #### 3. Protocol Versions: `minReaderVersion` &amp; `minWriterVersion`
...
# MAGIC %md
# MAGIC #### 4. Column Mapping — Enabling Column Renames
...
# MAGIC %md
# MAGIC #### 5. Table Properties Catalogue
...
# MAGIC %md
# MAGIC #### 6. `ALTER TABLE SET TBLPROPERTIES` — Modify Properties
...
# MAGIC %md
# MAGIC #### 7. `DESCRIBE DETAIL` — Complete Table Metadata
...
# MAGIC %md
# MAGIC ### Key Takeaways
# MAGIC - `SHOW TBLPROPERTIES` lists all configured properties; `ALTER TABLE SET TBLPROPERTIES` modifies them.
# MAGIC - **Protocol versions** (`minReaderVersion`, `minWriterVersion`) gate access to features like column mapping, deletion vectors, and CDF.
# MAGIC - `delta.columnMapping.mode = 'name'` enables column **renames without data rewrites** — critical for schema evolution in large tables.
# MAGIC
# MAGIC ### Self-Assessment Question
# MAGIC
# MAGIC *Q: You want to enable column renames and Change Data Feed on a new table. What properties must you set at creation time?*
# MAGIC
# MAGIC <details><summary>Click for answer</summary>
# MAGIC **A:** Set `delta.columnMapping.mode = 'name'` (or `'id'`), `delta.minReaderVersion = 2`, `delta.minWriterVersion = 5` (minimum for column mapping), and `delta.enableChangeDataFeed = true`. These must be set BEFORE writing data — you can&apos;t enable column mapping on an existing table without rewriting it.
# MAGIC </details>
...
# MAGIC
# MAGIC | # | Concept | Key Skill |
# MAGIC |---|---------|-----------|
# MAGIC | 1 | **ACID Transactions** | Atomic writes, optimistic concurrency, rollback on failure |
# MAGIC | 2 | **Transaction Log** | Inspecting `_delta_log`, understanding checkpoints, `DESCRIBE HISTORY` |
# MAGIC | 3 | **Time Travel** | `VERSION AS OF`, `TIMESTAMP AS OF`, `RESTORE TABLE` |
# MAGIC | 4 | **Schema Enforcement** | Type validation, `mergeSchema`, `overwriteSchema`, `autoMerge` |
# MAGIC | 5 | **Liquid Clustering** | `CLUSTER BY`, Z-Ordering, file skipping for queries |
# MAGIC | 6 | **OPTIMIZE & Compaction** | Small-file consolidation, `ZORDER BY`, read performance |
# MAGIC | 7 | **VACUUM & Lifecycle** | Storage cleanup, retention periods, DRY RUN, time travel impact |
# MAGIC | 8 | **Change Data Feed** | `table_changes()`, `_change_type`, incremental processing |
# MAGIC | 9 | **Deletion Vectors** | Soft-delete markers, physical vs logical deletes, GDPR implications |
# MAGIC | 10 | **Table Properties** | Protocol versions, column mapping, auto-optimize, `DESCRIBE DETAIL` |
# MAGIC
# MAGIC ### Delta Lake Commands You Can Now Use
# MAGIC
# MAGIC ```
# MAGIC -- Inspection
...
# MAGIC
# MAGIC -- Time Travel
# MAGIC SELECT * FROM table_name VERSION AS OF N
# MAGIC SELECT * FROM table_name TIMESTAMP AS OF '2024-01-01'
# MAGIC RESTORE TABLE table_name TO VERSION AS OF N
# MAGIC
# MAGIC -- Maintenance
...
# MAGIC SELECT * FROM table_changes('table_name', start_version)
# MAGIC SELECT * FROM table_changes('table_name', start, end)
# MAGIC
# MAGIC -- Configuration
# MAGIC ALTER TABLE table_name SET TBLPROPERTIES ('key' = 'value')
# MAGIC ALTER TABLE table_name RENAME COLUMN old_name TO new_name
# MAGIC ```
# MAGIC
# MAGIC ---
...
# MAGIC %md
# MAGIC ## Score Yourself: How Many Concepts Do You Understand?
# MAGIC
# MAGIC Check off each concept you feel confident with after completing this notebook.
# MAGIC
# MAGIC - [ ] **Concept 1: ACID Transactions** — I can explain atomic writes and optimistic concurrency
# MAGIC - [ ] **Concept 2: Transaction Log** — I understand `_delta_log` structure and checkpointing
# MAGIC - [ ] **Concept 3: Time Travel & RESTORE** — I can query past versions and roll back tables
# MAGIC - [ ] **Concept 4: Schema Enforcement vs. Evolution** — I know when to use mergeSchema vs overwriteSchema
# MAGIC - [ ] **Concept 5: Liquid Clustering** — I understand clustering vs partitioning and can use Z-Ordering
# MAGIC - [ ] **Concept 6: OPTIMIZE & File Compaction** — I can consolidate small files and ZORDER tables
# MAGIC - [ ] **Concept 7: VACUUM & Storage Lifecycle** — I know how to manage storage and retention
# MAGIC - [ ] **Concept 8: Change Data Feed** — I can read row-level changes with table_changes()
# MAGIC - [ ] **Concept 9: Deletion Vectors** — I understand soft-deletes and their performance benefits
# MAGIC - [ ] **Concept 10: Table Properties** — I can configure protocol versions, column mapping, and auto-optimize
# MAGIC
# MAGIC **Score:** ___ / 10
# MAGIC
# MAGIC ### Next Steps
# MAGIC
# MAGIC After mastering these fundamentals, continue to notebook `02_Delta_Lake_Advanced.py` for:
# MAGIC - MERGE operations and SCD Type 2
# MAGIC - Streaming with Delta (Structured Streaming)
# MAGIC - Generated columns and default values
# MAGIC - Partitioning strategies and predicate pushdown
# MAGIC - Data skipping and bloom filters
# MAGIC - Multi-cluster concurrent writes
# MAGIC - Delta Sharing basics
# MAGIC
# MAGIC ---
# MAGIC *Notebook created for Databricks Community Edition | Delta Lake Fundamentals #1-#10*
