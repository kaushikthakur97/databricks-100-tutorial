...
# MAGIC %md
# MAGIC # 06 — Performance & Cost Optimization
# MAGIC
# MAGIC **Concepts Covered:** #51–#60  
# MAGIC **Environment:** Databricks Community Edition (single-node, free)  
# MAGIC **Goal:** Master performance tuning and cost optimization techniques on Databricks.
# MAGIC
# MAGIC | # | Concept | Difficulty | Type |
# MAGIC |---|---------|------------|------|
# MAGIC | 51 | Serverless Compute Economics | Easy | Conceptual |
# MAGIC | 52 | Predicate Pushdown & Data Skipping | Medium | Hands-on |
# MAGIC | 53 | Predictive Optimization | Medium | Conceptual + Hands-on |
# MAGIC | 54 | Column Statistics & File Pruning | Medium | Hands-on |
# MAGIC | 55 | Write Optimization | Medium | Hands-on |
# MAGIC | 56 | Cluster Sizing & Autoscaling | Medium | Conceptual |
# MAGIC | 57 | DBU Cost Model & Billing | Medium | Conceptual + Hands-on |
# MAGIC | 58 | Query Profile & Execution Plans | Medium | Hands-on |
# MAGIC | 59 | Join Performance Diagnosis | Hard | Hands-on |
# MAGIC | 60 | MERGE Write Amplification | Hard | Hands-on |

# COMMAND ----------

# MAGIC %md
# MAGIC ## Concept 51 — Serverless Compute Economics
# MAGIC
# MAGIC **NOTE:** Serverless compute is **not available** in Databricks Community Edition. This section is conceptual.
# MAGIC
# MAGIC ### What is Serverless Compute?
# MAGIC
# MAGIC Serverless compute removes the need to manage clusters directly. You submit workloads, and Databricks provisions compute resources automatically in your account. You pay only for the resources consumed by your workload.
# MAGIC
# MAGIC ### DBU-Based Pricing Model
# MAGIC
# MAGIC Databricks pricing is based on **DBUs** (Databricks Units) — a normalized unit of processing capability per hour.
# MAGIC
# MAGIC | Compute Type | DBU/hr (approx.) | Billing Granularity | Best For |
# MAGIC |---|---|---|---|
# MAGIC | **Serverless SQL** | 0.70–1.10 | Per-second, 1-min minimum | BI dashboards, ad-hoc SQL |
# MAGIC | **Serverless Jobs** | 0.55–0.75 | Per-second, 1-min minimum | Notebook/scheduled jobs |
# MAGIC | **Classic SQL Warehouse** | 0.40–0.55 | Per-second, 10-min minimum | Steady-state SQL workloads |
# MAGIC | **Classic All-Purpose** | 0.40–0.55 | Per-second, 10-min minimum | Development, exploration |
# MAGIC | **Classic Jobs** | 0.30–0.40 | Per-second, 10-min minimum | Scheduled production pipelines |
# MAGIC
# MAGIC *Exact pricing varies by tier (Standard/Premium/Enterprise) and cloud provider.*

# COMMAND ----------

# MAGIC %md
# MAGIC ### Serverless vs Classic: When Each Wins
# MAGIC
# MAGIC **Serverless is cheaper when:**
# MAGIC - Workloads are **bursty** (intermittent, unpredictable)
# MAGIC - Many short-running queries or jobs
# MAGIC - You want zero cluster management overhead
# MAGIC - Start-up latency is important (serverless starts faster)
# MAGIC - Idle time between jobs is significant
# MAGIC
# MAGIC **Classic is cheaper when:**
# MAGIC - Workloads are **sustained** (24/7 processing)
# MAGIC - Jobs run continuously with little idle time
# MAGIC - You can right-size clusters precisely
# MAGIC - Using spot instances for deep discounts (30–50% off)
# MAGIC - Predictable, long-running ETL pipelines
# MAGIC
# MAGIC ### Elimination of Idle Cluster Waste
# MAGIC
# MAGIC With classic clusters:
# MAGIC - Clusters are always on (costing DBU/hr) even when idle
# MAGIC - Autoscaling helps but cannot go to zero
# MAGIC - Manual start/stop introduces latency
# MAGIC
# MAGIC With serverless:
# MAGIC - *No idle cost* — you pay only for active processing
# MAGIC - Compute scales down to zero between queries
# MAGIC - No cluster management overhead

# COMMAND ----------

# MAGIC %md
# MAGIC ### Conceptual Pricing Comparison
...
# MAGIC %md
# MAGIC ### Key Takeaways — Serverless
# MAGIC
# MAGIC | Factor | Serverless | Classic |
# MAGIC |---|---|---|
# MAGIC | Start-up time | 5–15 seconds | 3–7 minutes |
# MAGIC | Billing minimum | 1 minute | 10 minutes |
# MAGIC | Idle cost | $0 | DBU/hr × idle time |
# MAGIC | Spot instances | Not available | Available (30–50% less) |
# MAGIC | Cluster management | None | Required |
# MAGIC | Max scale | Elastic | Defined by config |
# MAGIC | Photon support | Yes | Yes |
# MAGIC

# COMMAND ----------

# MAGIC %md
# MAGIC ## Concept 52 — Predicate Pushdown & Data Skipping
# MAGIC
# MAGIC **Available in Community Edition — full hands-on.**
# MAGIC
# MAGIC ### Theory
# MAGIC
# MAGIC **Predicate pushdown** means filters are pushed down to the storage layer, so only relevant data is read from disk.  
# MAGIC **Data skipping** uses Delta Lake per-file statistics (min/max values) to skip entire files that can't match the filter.
# MAGIC
# MAGIC **What enables skipping:**
# MAGIC - Partition filters (direct path elimination)
# MAGIC - `ZORDER` clustering on filter columns
# MAGIC - Delta stats (min/max per file for first 32 columns)
# MAGIC
# MAGIC **What BREAKS pushdown:**
# MAGIC - Applying functions to filter columns (`YEAR(date_col) = 2023` instead of `date_col BETWEEN`)
# MAGIC - Casting (`CAST(col AS STRING) = 'value'`)
# MAGIC - Complex expressions that can't be evaluated at the file level

# COMMAND ----------

# MAGIC %md
# MAGIC ### Create Test Table with Many Files
...
# MAGIC %md
# MAGIC ### Demonstrate Predicate Pushdown with EXPLAIN
...
# MAGIC %md
# MAGIC ### Timing Comparison: Good vs Bad Predicates
...
# MAGIC %md
# MAGIC ### Show Delta Statistics with File Skipping
...
# MAGIC %md
# MAGIC ### Key Takeaways — Predicate Pushdown
# MAGIC
# MAGIC | Pattern | Effect | Example |
# MAGIC |---|---|---|
# MAGIC | Partition column filter | Entire partitions skipped | `WHERE region = 'North'` |
# MAGIC | Direct column comparison | File-level min/max stats used | `WHERE amount > 400` |
# MAGIC | Function on column | Stats cannot be used | `WHERE YEAR(date) = 2023` |
# MAGIC | Cast on column | Stats cannot be used | `WHERE CAST(id AS STRING) = '5'` |
# MAGIC | Range on column | Stats used effectively | `WHERE date BETWEEN 'A' AND 'B'` |
# MAGIC | `IN` clause | Converted to range checks | `WHERE region IN ('N','S')` |
# MAGIC

# COMMAND ----------

# MAGIC %md
# MAGIC ## Concept 53 — Predictive Optimization
# MAGIC
# MAGIC **NOTE:** Predictive Optimization is a **full-platform feature** (not available in Community Edition). The concept is explained here, and manual equivalents are demonstrated for CE.
# MAGIC
# MAGIC ### What Is Predictive Optimization?
# MAGIC
# MAGIC Predictive Optimization is a **managed service** that automatically runs maintenance operations on tables:
# MAGIC
# MAGIC | Operation | What It Does | Trigger |
# MAGIC |---|---|---|
# MAGIC | **OPTIMIZE** | Compacts small files, ZORDERs data | After 5+ writes to the table |
# MAGIC | **VACUUM** | Removes old file versions | Retained for 7 days (default) |
# MAGIC | **ANALYZE** | Refreshes column statistics | After significant data change |
# MAGIC
# MAGIC **Key characteristics:**
# MAGIC - Enabled by default on Unity Catalog managed tables (full platform)
# MAGIC - Monitored via `system.information_schema.predictive_optimizations_history`
# MAGIC - No user configuration required
# MAGIC - Zero performance impact on user queries
# MAGIC - No additional cost (included)

# COMMAND ----------

# MAGIC %md
# MAGIC ### Manual Maintenance (Community Edition Equivalent)
...
# MAGIC %md
# MAGIC ### When Manual Maintenance Is Still Needed (Even With Predictive Optimization)
# MAGIC
# MAGIC Predictive Optimization helps but doesn't eliminate the need for manual intervention:
# MAGIC
# MAGIC | Scenario | Why Manual |
# MAGIC |---|---|
# MAGIC | **Precise ZORDER** | Predictive Optimization chooses columns automatically; you may want specific ZORDER columns |
# MAGIC | **Custom VACUUM retention** | Default is 7 days; you may want longer for recovery |
# MAGIC | **Immediate maintenance** | Predictive Optimization runs on a schedule; you may need immediate compaction after a large write |
# MAGIC | **Tables with deletion vectors** | Manual REORG TABLE may be needed |
# MAGIC | **Non-managed tables** | Predictive Optimization only works on managed tables in Unity Catalog |
# MAGIC
# MAGIC ```python
# MAGIC # Manual OPTIMIZE with custom ZORDER
# MAGIC spark.sql("OPTIMIZE my_table ZORDER BY (date_col, category)")
# MAGIC
# MAGIC # Manual VACUUM with custom retention
# MAGIC spark.sql("VACUUM my_table RETAIN 168 HOURS")
# MAGIC
# MAGIC # Manual ANALYZE
# MAGIC spark.sql("ANALYZE TABLE my_table COMPUTE STATISTICS FOR ALL COLUMNS")
# MAGIC ```

# COMMAND ----------

# MAGIC %md
# MAGIC ## Concept 54 — Column Statistics & File Pruning
# MAGIC
# MAGIC **Available in Community Edition — hands-on.**
# MAGIC
# MAGIC ### How Delta Collects Statistics
# MAGIC
# MAGIC Delta Lake automatically collects **per-file min/max statistics** for the **first 32 columns** of each Parquet file. These statistics are stored in the Delta transaction log and enable:
# MAGIC
# MAGIC - **File-level pruning**: If `WHERE amount > 500` and a file's max amount is 300, the entire file is skipped.
# MAGIC - **No data read needed** to determine if a file matches — just check the metadata.
# MAGIC
# MAGIC **Column order matters**: Statistics are only kept for the first 32 columns in the schema. Columns beyond 32 don't have stats available for file skipping.
...
# MAGIC %md
# MAGIC ### ANALYZE TABLE to Refresh Statistics
...
# MAGIC %md
# MAGIC ### Demonstrate File Skipping with Column Statistics
# MAGIC
# MAGIC Filter on a column with statistics (col_01, within first 32) vs without (col_35, beyond 32).
...
# MAGIC %md
# MAGIC ### DESCRIBE DETAIL — Check Table Statistics
...
# MAGIC %md
# MAGIC ### Statistics Property Configuration
...
# MAGIC %md
# MAGIC ## Concept 55 — Write Optimization
# MAGIC
# MAGIC **Available in Community Edition — full hands-on.**
# MAGIC
# MAGIC ### Overview
# MAGIC
# MAGIC Write optimization techniques improve **read performance** by controlling output file size and layout:
# MAGIC
# MAGIC | Technique | What It Does |
# MAGIC |---|---|
# MAGIC | `optimizeWrite` | Coalesces small files before writing (auto-shuffle) |
# MAGIC | `maxRecordsPerFile` | Caps records per output file |
# MAGIC | `targetFileSize` | Desired file size (default 1GB for Parquet) |
# MAGIC | Repartition before write | Controls number of output files |
# MAGIC | Coalesce before write | Reduces files without shuffle |
# MAGIC | ZORDER optimization | Clusters related data in files |

# COMMAND ----------

# MAGIC %md
# MAGIC ### Create Test Table and Benchmark Different Write Strategies
...
# MAGIC %md
# MAGIC ### Compare File Counts and Read Performance
...
# MAGIC %md
# MAGIC ### Write Optimization Summary
...
# MAGIC %md
# MAGIC ## Concept 56 — Cluster Sizing & Autoscaling
# MAGIC
# MAGIC **NOTE:** Community Edition is **single-node only** (1 driver, 0 workers). This section is conceptual but includes Spark config guidance applicable to CE.
# MAGIC
# MAGIC ### Cluster Sizing Principles
# MAGIC
# MAGIC | Workload Type | Cores | Memory | Workers | Rationale |
# MAGIC |---|---|---|---|---|
# MAGIC | **SQL Analytics / BI** | 4–8 per worker | 2–4 GB/core | 2–20 | High concurrency, small queries |
# MAGIC | **ETL / Data Engineering** | 8–16 per worker | 2–4 GB/core | 2–50 | CPU-intensive, large shuffles |
# MAGIC | **ML Training** | 8–16 per worker | 4–8 GB/core | 2–20 | GPU optional, memory for models |
# MAGIC | **Streaming** | 4–8 per worker | 4 GB/core | 2–10 | Sustained usage, low latency |
# MAGIC | **Development / Exploration** | 4 per worker | 8 GB/core | 1–2 | Interactive, low cost |
# MAGIC
# MAGIC ### The `spark.sql.shuffle.partitions` Rule
# MAGIC
# MAGIC Default: 200 partitions per shuffle operation.  
# MAGIC **Rule of thumb:** Set to 2–3× total cores in cluster.
# MAGIC
# MAGIC | Cluster Size | Total Cores | Recommended shuffle.partitions |
# MAGIC |---|---|---|
# MAGIC | 1 worker × 4 cores | 4 | 8–12 |
# MAGIC | 4 workers × 8 cores | 32 | 64–96 |
# MAGIC | 10 workers × 16 cores | 160 | 320–480 |

# COMMAND ----------

# MAGIC %md
# MAGIC ### Autoscaling Configuration
...
# MAGIC %md
# MAGIC ### Right-Sizing for Community Edition
# MAGIC
# MAGIC In Community Edition, you have a single node. Key Spark configurations to tune:
...
# MAGIC %md
# MAGIC ## Concept 57 — DBU Cost Model & Billing
# MAGIC
# MAGIC **NOTE:** System tables for billing are not available in Community Edition. This section explains the DBU model conceptually with cost calculation examples you can apply.
# MAGIC
# MAGIC ### What Is a DBU?
# MAGIC
# MAGIC **DBU** = Databricks Unit — the unit of measure for Databricks capacity consumption.
# MAGIC
# MAGIC **1 DBU = 1 hour of processing on a standardized unit of compute.**
# MAGIC
# MAGIC The actual cost per DBU depends on:
# MAGIC - **Tier**: Standard, Premium, or Enterprise
# MAGIC - **SKU**: All-Purpose Compute, Jobs Compute, SQL Compute, Serverless
# MAGIC - **Cloud provider**: AWS, Azure, GCP (prices vary slightly)
...
# MAGIC %md
# MAGIC ### Cost Calculation Examples
...
# MAGIC %md
# MAGIC ### Tagging Jobs for Cost Allocation
...
# MAGIC %md
# MAGIC ### Budget Policies (Full Platform Feature)
...
# MAGIC %md
# MAGIC ## Concept 58 — Query Profile & Execution Plans
# MAGIC
# MAGIC **Available in Community Edition — full hands-on with EXPLAIN.**
# MAGIC
# MAGIC ### Understanding EXPLAIN Output
# MAGIC
# MAGIC `EXPLAIN` shows how Spark will execute a query. There are three plan levels:
# MAGIC
# MAGIC | Plan | What It Shows |
# MAGIC |---|---|
# MAGIC | **Parsed Logical Plan** | What you wrote — unresolved references |
# MAGIC | **Analyzed Logical Plan** | Resolved column names and types |
# MAGIC | **Optimized Logical Plan** | After Catalyst optimizer applied rules |
# MAGIC | **Physical Plan** | Actual execution strategy (how Spark runs it) |
...
# MAGIC %md
# MAGIC ### Simple EXPLAIN — See the Physical Plan
...
# MAGIC %md
# MAGIC ### EXPLAIN EXTENDED — Full Plan Details
...
# MAGIC %md
# MAGIC ### Walking Through Plan Nodes
...
# MAGIC %md
# MAGIC ### Analyze a Complex Query Plan Step by Step
...
# MAGIC %md
# MAGIC ### Identifying Slow Operators
...
# MAGIC %md
# MAGIC ### Generate and Analyze Your Own Query Profile
...
# MAGIC %md
# MAGIC ## Concept 59 — Join Performance Diagnosis
# MAGIC
# MAGIC **Available in Community Edition — full hands-on.**
# MAGIC
# MAGIC ### Join Strategies in Spark
# MAGIC
# MAGIC | Strategy | When Used | Cost | Best For |
# MAGIC |---|---|---|---|
# MAGIC | **Broadcast Hash Join (BHJ)** | One side < threshold (default 10MB) | No shuffle | Small lookup tables |
# MAGIC | **Sort Merge Join (SMJ)** | Both sides large, join keys sortable | Shuffle both sides | Large-large joins |
# MAGIC | **Shuffled Hash Join (SHJ)** | One side 3× smaller than other | Shuffle one side | Medium-large joins |
# MAGIC | **Broadcast Nested Loop Join (BNLJ)** | Non-equi join, one side small | Cartesian risk | Cross/custom joins |
# MAGIC | **Cartesian Product Join** | No join condition | Very expensive | Avoid! |

# COMMAND ----------

# MAGIC %md
# MAGIC ### Create Large Tables for Join Benchmarking
...
# MAGIC %md
# MAGIC ### Test 1: Default Join (Auto Broadcast)
...
# MAGIC %md
# MAGIC ### Test 2: Sort-Merge Join (Disable Broadcast, Force Shuffle)
...
# MAGIC %md
# MAGIC ### Test 4: Join After Pre-Aggregation (Reduce Shuffle)
...
# MAGIC %md
# MAGIC ### Join Performance Summary
...
# MAGIC %md
# MAGIC ## Concept 60 — MERGE Write Amplification
# MAGIC
# MAGIC **Available in Community Edition — full hands-on.**
# MAGIC
# MAGIC ### What Is Write Amplification?
# MAGIC
# MAGIC When MERGE matches rows in a file, it **rewrites the ENTIRE file**, not just the matched rows. If 1 row in a 100MB file is updated, MERGE rewrites all 100MB. This is the core of write amplification.
# MAGIC
# MAGIC **Deletion Vectors** (Delta 3.0+, full platform) solve this by recording which rows are deleted/matched, avoiding full file rewrites. Community Edition uses an older Delta version without this feature.

# COMMAND ----------

# MAGIC %md
# MAGIC ### Create Test Table for MERGE Experiments
...
# MAGIC %md
# MAGIC ### MERGE 1: Wide MERGE (Matches Many Files — High Amplification)
...
# MAGIC %md
# MAGIC ### Reset Table for Second Test
...
# MAGIC %md
# MAGIC ### MERGE 2: Targeted MERGE (Single Product — Low Amplification)
...
# MAGIC %md
# MAGIC ### MERGE 3: DELETE + INSERT Approach (Alternative to MERGE)
...
# MAGIC %md
# MAGIC ### Analyze MERGE Performance with DESCRIBE HISTORY
...
# MAGIC %md
# MAGIC ### MERGE 4: MERGE with Matching on Clustered Keys (Optimization)
...
# MAGIC %md
# MAGIC ### MERGE Amplification Summary
...
# MAGIC %md
# MAGIC ## Performance & Cost — Summary Table
...
# MAGIC
# MAGIC Test your understanding of concepts #51–#60:
# MAGIC
# MAGIC **Concept 51 — Serverless Economics:**
# MAGIC 1. When is serverless more expensive than classic compute?
# MAGIC 2. What is the minimum billing period for serverless vs classic SQL warehouses?
# MAGIC 3. How does serverless eliminate idle cluster waste?
# MAGIC
# MAGIC **Concept 52 — Predicate Pushdown:**
# MAGIC 4. Why does `WHERE YEAR(date_col) = 2023` prevent predicate pushdown?
# MAGIC 5. What must you do to enable file-level statistics for a column?
# MAGIC 6. Does repartitioning to a different column break data skipping?
# MAGIC
# MAGIC **Concept 53 — Predictive Optimization:**
# MAGIC 7. What three operations does Predictive Optimization run automatically?
# MAGIC 8. When would you still manually run OPTIMIZE even with Predictive Optimization?
# MAGIC
# MAGIC **Concept 54 — Column Statistics:**
# MAGIC 9. For how many columns does Delta collect per-file statistics by default?
# MAGIC 10. What command refreshes column statistics?
# MAGIC
# MAGIC **Concept 55 — Write Optimization:**
# MAGIC 11. What's the difference between `coalesce()` and `repartition()` before writing?
# MAGIC 12. What does `maxRecordsPerFile` control?
# MAGIC
# MAGIC **Concept 56 — Cluster Sizing:**
# MAGIC 13. What's the recommended `spark.sql.shuffle.partitions` relative to core count?
# MAGIC 14. When would you use spot instances?
# MAGIC
# MAGIC **Concept 57 — DBU Cost Model:**
# MAGIC 15. Which SKU is cheapest per DBU for scheduled jobs?
# MAGIC 16. How do you allocate costs to specific teams?
# MAGIC
# MAGIC **Concept 58 — Query Plans:**
# MAGIC 17. What's the difference between a logical plan and a physical plan?
# MAGIC 18. What does an Exchange node in the physical plan indicate?
# MAGIC
# MAGIC **Concept 59 — Join Performance:**
# MAGIC 19. What join strategy does Spark use when one table is smaller than 10MB?
# MAGIC 20. How can you reduce shuffle during a join?
# MAGIC
# MAGIC **Concept 60 — MERGE Amplification:**
# MAGIC 21. Why does MERGE rewrite entire files?
# MAGIC 22. What technique reduces the number of files touched by MERGE?
# MAGIC 23. What Delta 3.0+ feature eliminates merge write amplification entirely?

# COMMAND ----------

# MAGIC %md
# MAGIC ### Quick Knowledge Check — Write Your Answers Below
...
# MAGIC %md
# MAGIC ## Performance Optimization Cheat Sheet
